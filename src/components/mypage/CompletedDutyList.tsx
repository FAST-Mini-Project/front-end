...
import styles from './CompletedDutyList.module.scss'
...
const CompletedDutyList: React.FC<MyDutyProps> = ({ dutyData, selectedYear, selectedMonth }) => {
    ...
    <section className={styles.list__container}>
      ...
      <div className={styles.list__body}>
        {sortedByDate.length === 0 ? (
          <p className={styles.list__none}>해당 기간에 아직 완료한 당직 일정이 없습니다.</p>
        ) : (
          <ul>
            {sortedByDate.map((duty) => (
              <li key={duty.dutyId} title="이미 근무한 당직 일정입니다." className={styles.list__item}>
                <span>{duty.date}</span>
              </li>
            ))}
          </ul>
...
export default CompletedDutyList
